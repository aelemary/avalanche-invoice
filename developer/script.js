...
function setPaymentMarkup(method) {
  const common = '<label class="full">Account holder name<div class="account-holder-input"><input name="accountHolderPrefix" maxlength="16" placeholder="Title" aria-label="Account holder title"><input name="accountHolder" required></div></label>';
...
function renderInvoice(invoiceNumberOverride) {
  ...
  context.font = '700 22px Inter, Arial'; context.fillText('Payment details', left, y); y += 32; const paymentLines = Object.entries(data.payment).filter(([key, value]) => key !== 'method' && key !== 'accountHolderPrefix' && value).map(([key, value]) => key === 'accountHolder' ? `Account Holder: ${[data.payment.accountHolderPrefix, value].filter(Boolean).join(' ')}` : `${key.replace(/([A-Z])/g, ' $1').replace(/^./, char => char.toUpperCase())}: ${value}`); y = drawLines(paymentLines, left, y, { font: '23px Inter, Arial', lineHeight: 33 }) + 25;
